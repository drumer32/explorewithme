...
public class EventDtoMapper {
    ...
    public Event newEventDtoToEvent(Integer userId, NewEventDto newEventDto) {
        Event event = mapperConfig.map().map(newEventDto, Event.class);
        event.setInitiator(userId);
        return event;
    }

    public EventFullDto eventToFullDto(Event event) {
        EventFullDto eventFullDto = mapperConfig.map().map(event, EventFullDto.class);
        UserShortDto initiator = mapperConfig.map().map(event.getInitiator(), UserShortDto.class);
        eventFullDto.setInitiator(initiator);
        return eventFullDto;
    }

    public EventShortDto eventToShortEventDto(Event event) {
        return mapperConfig.map().map(event, EventShortDto.class);
    }
}
