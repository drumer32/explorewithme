...
public class PrivateEventController {
    ...
    public List<Integer> getEvents(@PathVariable Integer userId,
    ...
    public EventFullDto createEvent(@PathVariable Integer userId,
                                    @RequestBody NewEventDto newEventDto) throws BadConditionException {
        if (newEventDto.getEventDate().isAfter(LocalDateTime.now().plusHours(2))) {
            return mapper.eventToFullDto(eventService.addEvent(mapper.newEventDtoToEvent(userId ,newEventDto)));
